...
$(function(){
	...
	$("#submit_camera").click(function(event){
		...
		/*
		try{
			var result=window.flashContent.comparePhoto(window.location.protocol+"//"+window.location.host+":"+(window.location.port?window.location.port:(window.location.protocol=="https:"?443:80))+"/user/compareFace");
			if(result){
				$("#face_camera").fadeIn(100).addClass("loading");
			}else{
				$("#face_camera").fadeOut(100).removeClass("loading");
				$(".camera_result").addClass("fail").fadeIn(100);
				$(".faceCompareTip").text("当前无法提交识别,请重试").addClass("fail");
			}
		}catch(e){
			$("#face_camera").fadeOut(100).removeClass("loading");
			$(".camera_result").addClass("fail").fadeIn(100);
		...
		setTimeout(function(){
			compareCallback(1,"{status:'success',data:{isPass:1}}");
		},2000)
	});
...
function compareCallback(_result,_data){
	...
	if(json && json.status==="success" && json.data.isPass){
		$(".camera_result").removeClass("fail").fadeIn(100);
		$(".faceCompareTip").text("认证通过").removeClass("fail");
		$("#face_camera").fadeOut(100);
		setTimeout(function(){
			if(window.parent && typeof(window.parent[urlRequest.callback])==="function"){
	...
	}else if(json && json.status==="success" && !json.data.isPass){
		$("#face_camera").fadeOut(100);
		$(".camera_result").addClass("fail").fadeIn(100);
		$(".faceCompareTip").text("认证未通过").addClass("fail");
		if(window.parent && typeof(window.parent[urlRequest.callback])==="function"){
		...
		$("#face_camera").fadeOut(100);
		$(".camera_result").addClass("fail").fadeIn(100);
		$(".faceCompareTip").text("抱歉,认证过程中发生错误").addClass("fail");
		if(window.parent && typeof(window.parent[urlRequest.callback])==="function"){
			window.parent[urlRequest.callback](false);
		}
	}
